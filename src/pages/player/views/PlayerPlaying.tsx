      ...
      {categories.map((cat, idx) => (
        ...
        >
          <div className="flex items-center justify-between ml-2 mb-2">
            <label className="text-[10px] text-hacker-green/80 font-black font-mono uppercase tracking-[0.3em]">
              &gt; {idx + 1}. {cat}
            </label>
            <button
              onClick={() => toggleJoker(cat)}
              disabled={isLocked}
              className={`px-3 py-1 text-[9px] font-black tracking-widest uppercase transition-all flex items-center gap-1 border-l-2
                ${
                  jokerCategory === cat
          ...
          <div className="relative group">
            <motion.input
              ...
              className={`w-full bg-black/80 border-2 rounded-sm px-5 py-4 md:px-8 md:py-5 font-mono font-bold transition-all disabled:opacity-50 text-lg md:text-xl cyber-panel focus:outline-none
                ${
                  jokerCategory === cat 
