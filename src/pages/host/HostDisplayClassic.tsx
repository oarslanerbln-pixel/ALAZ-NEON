...
/**
 * Klasik HENGAME ARENA (scattegories) modu. HostDisplay.tsx'ten kendi
 * dosyasına çıkarıldı ki lazy-load edilebilsin: önceden bütün oyun modlarının
 * kodu (Quiz/Bomba/Sensör/Çark/Overload/Klasik) tek bir ~765KB'lık pakette
 * birleşiyordu — host TV'si o an oynanmayan 5 modun kodunu da indirmek
 * zorunda kalıyordu. Artık her mod kendi parçasında, sadece seçilen mod
 * indiriliyor (bkz. HostDisplay.tsx'teki lazy() + Suspense).
 */
export function HostDisplayClassic({
  roomId,
  room,
  players,
  submittedPlayerIds,
  ...
  const proceedFromStandings = async () => {
    ...
    if (room.current_round >= room.total_rounds) {
      ...
      grantGameRewards(room.game_mode, players, venue).catch((err) =>
        console.error("[HostDisplayClassic] Ödül dağıtımı başarısız:", err),
      );
