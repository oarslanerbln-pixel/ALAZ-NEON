...
export function HostIntro({ players, onComplete }: HostIntroProps) {
  ...
  // Stable ref for onComplete — prevents timeout resets when parent re-renders
  ...
  useEffect(() => {
    // 5. WOW Cinematic Phase — uses ref so this effect only runs when phase changes
    if (phase === "wow") {
  ...
  }, [phase]); // ← onComplete REMOVED from deps — ref ensures latest callback is always called
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-black fixed inset-0 z-[100] overflow-hidden noise-suppression font-mono">
      ...
      <button
        ...
        className="absolute top-6 right-6 z-50 px-4 py-2 bg-black/60 hover:bg-white/10 border border-white/20 hover:border-alaz-orange text-gray-400 hover:text-white text-xs tracking-widest uppercase rounded-sm transition-all flex items-center gap-2 cursor-pointer backdrop-blur-md"
      >
        <span>GEÇ</span>
        <span>&gt;&gt;</span>
      </button>
