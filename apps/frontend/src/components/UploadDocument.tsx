  ...
  return (
    <div className="flex flex-col gap-6 items-center w-full">
      <div className="flex flex-col gap-2 w-full mb-2">
        <h2 className="text-lg font-bold">Dil Seçimi / Language / اللغة</h2>
        <div className="flex gap-2">
          <button
            onClick={() => setLanguage('tr')}
            className={`flex-1 p-3 rounded-lg font-bold focus-visible:ring-4 focus-visible:ring-yellow-400 focus-visible:outline-none transition-colors border-2 ${language === 'tr' ? 'bg-blue-600 border-blue-400' : 'bg-gray-800 border-gray-600 hover:bg-gray-700'}`}
          >
            Türkçe
          </button>
          <button
            onClick={() => setLanguage('en')}
            className={`flex-1 p-3 rounded-lg font-bold focus-visible:ring-4 focus-visible:ring-yellow-400 focus-visible:outline-none transition-colors border-2 ${language === 'en' ? 'bg-blue-600 border-blue-400' : 'bg-gray-800 border-gray-600 hover:bg-gray-700'}`}
          >
            English
          </button>
          <button
            onClick={() => setLanguage('ar')}
            className={`flex-1 p-3 rounded-lg font-bold focus-visible:ring-4 focus-visible:ring-yellow-400 focus-visible:outline-none transition-colors border-2 ${language === 'ar' ? 'bg-blue-600 border-blue-400' : 'bg-gray-800 border-gray-600 hover:bg-gray-700'}`}
          >
            العربية
          </button>
        </div>
