  ...
  return (
    <div className="flex flex-col gap-6 items-center w-full">
      <div className="flex flex-col w-full gap-2">
        <h2 className="text-xl font-bold flex items-center gap-2"><Globe size={24} /> Çeviri Dili Seçin</h2>
        <div className="grid grid-cols-3 gap-2 w-full">
          {(['tr', 'en', 'ar'] as const).map((lang) => (
             <button
                key={lang}
                onClick={() => setLanguage(lang)}
                className={`p-4 rounded-xl font-bold text-lg border-2 transition-colors focus-visible:ring-4 focus-visible:ring-yellow-400 focus-visible:outline-none ${language === lang ? 'bg-blue-600 border-blue-400' : 'bg-gray-800 border-gray-600 hover:bg-gray-700'}`}
             >
      ...
      {result && (
        <div className="w-full bg-gray-800 p-6 rounded-xl mt-4" role="status" aria-live="polite">
          ...
          <p className="text-sm font-bold text-yellow-400 border-t border-gray-600 pt-4 mt-4">
