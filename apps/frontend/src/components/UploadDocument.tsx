    ...
    <div className="flex flex-col gap-6 items-center w-full">
      <div className="flex justify-center gap-4 w-full">
        {['TR', 'EN', 'AR'].map((lang) => (
          <button
            key={lang}
            onClick={() => setLanguage(lang as 'TR' | 'EN' | 'AR')}
            className={`px-4 py-2 rounded-xl font-bold border-2 focus-visible:ring-4 focus-visible:ring-yellow-400 focus-visible:outline-none ${language === lang ? 'bg-yellow-400 text-black border-yellow-400' : 'bg-gray-800 border-gray-600 hover:bg-gray-700'}`}
          >
            {lang}
          </button>
        ))}
      ...
      {result && (
        <div className="w-full bg-gray-800 p-6 rounded-xl mt-4" role="status" aria-live="polite">
          ...
          <p className="mt-6 text-base text-yellow-400 font-bold border-t border-gray-700 pt-4">
