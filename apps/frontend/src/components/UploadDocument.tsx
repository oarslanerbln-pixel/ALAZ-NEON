  ...
  return (
    <div className="flex flex-col gap-6 items-center w-full">
      <div className="w-full flex gap-2 justify-center mb-2">
        <button onClick={() => setLanguage('tr')} className={`px-4 py-2 rounded-lg font-bold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-yellow-400 ${language === 'tr' ? 'bg-blue-600' : 'bg-gray-700'}`}>TR</button>
        <button onClick={() => setLanguage('en')} className={`px-4 py-2 rounded-lg font-bold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-yellow-400 ${language === 'en' ? 'bg-blue-600' : 'bg-gray-700'}`}>EN</button>
        <button onClick={() => setLanguage('ar')} className={`px-4 py-2 rounded-lg font-bold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-yellow-400 ${language === 'ar' ? 'bg-blue-600' : 'bg-gray-700'}`}>AR</button>
      </div>
      ...
      {result && (
        <div role="status" aria-live="polite" className="w-full bg-gray-800 p-6 rounded-xl mt-4">
          ...
          <p className="text-base text-yellow-400 font-bold border-t border-gray-700 pt-4 mt-2">Bu bir tıbbi tavsiye değildir, yalnızca dil sadeleştirme aracıdır. Lütfen doktorunuza danışın.</p>
        </div>
      )}
    </div>
  );
